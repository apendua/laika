...
function ClientConnector(webDriverUrl, options) {
  ...
  self._browser.init(function (err) {
    if (err) {
      ...
      //}
      //if (options.viewport) {
      //  self._browser.setWindowSize(options.viewport.width, options.viewport.height);
      //}
    }
  ...
  this.sendEvent = function sendEvent () {
    var args = Array.prototype.slice.call(arguments, 0);
    if (args.length > 4) {
      throw new Error('Currently `sendEvent` can only accept at most 4 arguments.');
    }
    // make sure that the callback is the last argument
    if (typeof args[args.length - 1] === 'function') {
      while (args.length < 4) {
        args.splice(args.length - 1, 0, null);
      }
    }
    // make sure we don't fire the event too soon
    if (pageOpened) {
